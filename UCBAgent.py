...
class UCBAgent:
    def __init__(self, choices, exploration_factor=1.0):
    ...
    def present(self, context, choices):
        ...
        if self.total_rounds <= len(self.choices):  # Assurez-vous que chaque bras est tiré au moins une fois
            chosen_arm = choices[self.total_rounds - 1]
        else:
            ucb_values = {choice: val + self.exploration_factor * math.sqrt(
                math.log(self.total_rounds) / self.number_of_selections[choice]) for choice, val in
                          self.estimated_values.items()}
